...
| 72 | Post-migration CLI silently hits the OLD AWS account — deploys/wait-amplify act on the wrong account | After migrating a project to a NEW AWS account, the shell `default` profile (or a shared profile) still points at the old account. Prefix every aws/amplify command with the new project profile and confirm with `aws sts get-caller-identity`; symptom: wait-amplify loops printing `job ? status ?` (list-jobs returns empty cross-account). Document account + profile in the project's AGENTS.md deploy section |
| 73 | Alarms/monitoring silently missing after deploy — SNS subscription and RDS alarms never created | Monitoring resources are gated on deploy-time env vars (e.g. `ALARM_EMAIL`, `RDS_DB_IDENTIFIER`) via CFN conditions; a deploy without them skips them BY DESIGN. Always deploy with the full env block from `.env.example` |
| 74 | Days wasted auditing a "missing" domain that never existed | A one-letter typo propagates through docs (e.g. `maybellabs.my` vs `mabellabs.my`); WHOIS was checked against the wrong name while the real domain sat healthy on placeholder DNS. Before declaring a domain unregistered: cross-check spelling against the registrar panel AND the Amplify domain record AND what DNS records actually exist (placeholder A records = parked, not broken) |
| 75 | `update-user-pool-client` silently drops unlisted callback URLs too (on top of #25's auth-flow reset) | Fetch existing CallbackURLs/LogoutURLs FIRST and append new domains rather than replacing — otherwise localhost dev logins or an old storefront URL silently break; re-pass `--explicit-auth-flows` in the same command |
| 76 | "Press back and the search filter is gone" — one symptom, TWO independent causes | (1) Client-side filter state dies on navigation: mirror it into the URL (`?q=`) using `router.replace`, NOT `push` (a history entry per keystroke makes back useless), and read it back AFTER hydration — never in `useState`'s initializer, or the static export's prerendered markup (which has no query) mismatches and hydration breaks. (2) The in-app Back control must not GUESS its destination: `history.state.idx` is null in current App Router versions and `document.referrer` never updates on client-side navigation, so both silently fall through to `/`. Carry the listing URL on the card link (`?from=`) and follow it — accepting only internal paths (`startsWith("/") && !startsWith("//")`) so a shared link cannot redirect off-site |
| 77 | Amplify build reports **SUCCEED** while quietly shipping fallback/mock data — real pages go blank or 404 and nobody is told | Static-export pages are prerendered by reading the live API at build time, and the fetch layer deliberately falls back to mock fixtures when a read fails. A transient 5xx during the build therefore produces a GREEN build full of fake data — or empty shells for `[slug]` routes. Seen when the BACKEND was deployed while the frontend build was in flight: the deploy's brief 5xxs on detail endpoints blanked 5 pages and the job still reported success. Never deploy the backend during an Amplify build; grep the build log for the fallback warning (any hit = a page is wrong); verify page BODY (title, image count, size), never HTTP status, since an empty shell still answers 200. Rebuild to repair — and re-check, it is intermittent, so one clean build is not proof |
| 78 | Filtered results render but **paint nothing** — cards present, clickable, correct height, images loaded, yet invisible; reported by the owner as "images not showing" | A scroll reveal that drives its children from the PARENT's `whileInView` + `viewport={{ once: true }}` never re-propagates its variant state to children that mount LATER. Filtering swaps a grid's cards without the group ever leaving the viewport, so the replacements mount at the `hidden` variant (opacity 0) and stay there — the grid takes its full height and nothing paints. The DOM looks perfectly healthy (`naturalWidth` set, `complete: true`, in viewport), which is exactly why it gets misdiagnosed as a broken or lazy image; it also looks intermittent, because it only starts once the one-shot reveal has already fired. Fix: track the in-view state with `useInView` and drive the group with `animate` so late children inherit `show`. Assert on computed `opacity` in tests, never on DOM presence |
